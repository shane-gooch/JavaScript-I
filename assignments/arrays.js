...
let filteredArr = inventory.filter((car) => {
    if (car.id === 33) {
        return true;
    }
    return false;
...
let lastCar = 0;
console.log(`The last car is ${inventory[inventory.length-1].car_make} ${inventory[inventory.length-1].car_model}.`);
...
let BMWAndAudi = [];
// let filteredArray = inventory.filter((element) => {
//     if(inventory.car_make === 'BMV' || inventory.car_make === "Audi"){
//         BMWAndAudi.push(filteredArray);
//     }
// })
for(let i = 0; i < inventory.length; i++) {
